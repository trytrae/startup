...
def create_user_system_message(profile):
    history_purchases = "\n            ".join([f"{i+1}. {item}" for i, item in enumerate(profile["历史购买"])])
    return f"""你是一位在{profile["城市"]}工作的{profile["职业"]},日常喜欢{profile["生活方式"]},有一个{profile["孩子年龄"]}的孩子,
            每年在童装上的花费是{profile["童装花费"]}。你购买过的童装包括:
            {history_purchases}
...
def conduct_feedback(model="qwen-vl-max", chat_turn_limit=50, image_path=None):
    ...
    for profile in get_user_profiles():
        print(f"\n开始与{profile['城市']}的{profile['职业']}妈妈对话...")
        ...
        role_play_session = RolePlaying(
            ...
            user_agent_kwargs=dict(
                model=model,
                system_message=create_user_system_message(profile) + """
...
if __name__ == "__main__":
    ...
    for result in feedback_results:
        ...
        print(f"\n{profile['城市']}的{profile['职业']}妈妈的反馈摘要:")
        print("-" * 50)
        # 这里可以添加反馈分析逻辑